...
for cam_idx in range(n_cameras):
    ...
    for fname in files:
        ...
        # frame_idxs[i] = [orig. full filename, orig. frame number]
        frame_idxs.append([m.group(0), int(m.group(1))])

    frame_idxs.sort(key=lambda x: x[1])

    section1 = list(filter(lambda x: x[1] >= 70_000 and x[1] < 71_000, frame_idxs))
    section2 = list(filter(lambda x: x[1] < 70_000 or x[1] >= 71_000, frame_idxs))

    frame_idxs = []
    frame_idxs.extend(section2)
    frame_idxs.extend(section1)

    for i, frame_idx in enumerate(frame_idxs):
        source = Path(frame_idx[0])
        target = Path(project_dir, "frames_fixed", f"cam{cam_idx}_{i:03d}.png")
