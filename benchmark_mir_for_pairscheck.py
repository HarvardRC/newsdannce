...
def make_path(cam_no):
    return f"G:/Videos/6cam/jn187/2024_04_27_chris_test_mir_intrinsic/edge_validation/Camera{cam_no}/0.mp4" #test with new edge but not recognizible
    # return f"G:/Videos/6cam/jn187/2024_04_23_chris_test/extrinsic_test_1/Camera{cam_no}/0.mp4"
...
# params = [
#     "G:/Videos/6cam/jn187/2024_04_23_chris_test/hires_cam1_params.mat",
...
for i in range(n_cams):
    for j in range(n_cams-1):
        ...
        for camera_idx in range(n_cams):
            ...
            # )
            # cv2.imwrite(img=draw_img, filename=f"./out/triang_pairs/cam{camera_idx}.png")
            # os.makedirs("./out/triang_pairs", exist_ok=True)
            # print("Saving to: ", f"./out/triang_pairs/cam{camera_idx}.png")
            # print("RPE is: ", rpe)
            rpes[camera_idx].append(rpe)

with open(os.path.join(base_path, 'out/240427_edge_check_rpes.csv'), 'w', newline='') as csvfile:
