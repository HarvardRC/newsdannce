...
for kk = 1 : numcams
    ...
    [imagePoints, boardSize, imagesUsed] = detectCheckerboardPoints(cat(4, video_base{imUse1}));
  %  squareSize = 16.57; %mm
...
for kk = 1 : numcams
    [worldOrientation{kk}, worldLocation{kk}] = estimateWorldCameraPose(...
            double(point_coordinates{kk}), double(LFrame_coordinates), params_individual{kk}, ...
            'Confidence', 95, 'MaxReprojectionError', 4, 'MaxNumTrials', 5000);
...
fprintf(['starting checkerboard analysis:\n'...
        ...
        'If this is a problem, know that you dont need to click on the L exactly, but can exaggerate the positions. '...
        'In the code, consider making shortdim equal to the opposite of longdim'])
%cam1 =num2str(basecam');
%cam2 = {'2','2','3','4','5','6'};
%params_comp = cell(1,numel(cam1));
%params_comp{kk} = params;
...
for kk=1:numcams
    ...
    [worldOrientation_2{kk}, worldLocation_2{kk}] = estimateWorldCameraPose( ...
            double(allimagepoints{kk}), double(location3d), params_individual{kk}, ...
            'Confidence', 95, 'MaxReprojectionError', 4, 'MaxNumTrials', 5000);
...
fprintf('saving world coordinates \n')
save( worldcoordinates_framename_2, 'params_individual', 'worldOrientation_2', ...
        'worldLocation_2', 'rotationMatrix_2', 'translationVector_2');
