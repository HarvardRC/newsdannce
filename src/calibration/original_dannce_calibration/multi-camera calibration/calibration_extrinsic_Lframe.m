...
for kk = 1 : numel(lframe)
            ...
            for mn = 1e10 
                try
                    [worldO, worldL] = estimateWorldCameraPose( ...
                            ...
                            'Confidence', c, 'MaxReprojectionError', mr, 'MaxNumTrials', mn);
    ...
    for i=1:200
        try
            [worldO, worldL] = estimateWorldCameraPose(...
                double(point_coordinates{kk}), double(LFrame_coordinates), params_individual{kk}, ...
                'Confidence', c_save, 'MaxReprojectionError', mr_save, 'MaxNumTrials', mn_save);
            [rotationM, translationV] = cameraPoseToExtrinsics(worldO, worldL);
            imagePoints = worldToImage(params_individual{kk}, rotationM, translationV, double(LFrame_coordinates));
            err = mean(sqrt(sum((imagePoints-point_coordinates{kk}) .^ 2, 2)));
            if err < curr_err
