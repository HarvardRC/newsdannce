...
@celery_app.task
def import_video_folder_worker(video_folder_id):
    start = time.time()
    # 1. get path name from database

    with get_db_context() as conn:
        curr = conn.cursor()
        curr.execute("BEGIN")
        row = curr.execute(
            f"SELECT status, name, path, src_path FROM {TABLE_VIDEO_FOLDER} WHERE id=?",
            (video_folder_id,),
        ).fetchone()
        row = dict(row)
        if row["status"] != "PENDING":
            logger.warning(
                f"Status must be pending for import_video_folder_worker: id: {video_folder_id}; status={row['status']}"
            )
            raise Exception("Invalid status import video folder")
        resource_path = row["path"]
        video_folder_path_src = Path(row["src_path"])
        video_folder_path_dest = Path(settings.VIDEO_FOLDERS_FOLDER, resource_path)

        # 2. update database row to PROCESSING status
        logger.info(
            f"Processing video folder id: {video_folder_id}. Src={video_folder_path_src}. Dest={video_folder_path_dest}"
        )
        curr.execute(
            f"UPDATE {TABLE_VIDEO_FOLDER} SET status='PROCESSING' WHERE id=?",
            (video_folder_id,),
        )
        curr.execute("COMMIT")
    ...
    try:
        ...
        # 3. copy and reencode files
        _copy_reencode_video_folder(video_folder_path_src, video_folder_path_dest)

        # 4. identify calibration parameters and any label3d files
        ret = _identify_copy_calibration_params(
            video_folder_path_src, video_folder_path_dest
        )
        params = ret["params"]
        dannce_label_file = ret["dannce_label_file"]
        com_label_file = ret["com_label_file"]

        # 5. update database after copying is done
        _update_db_completed(
            video_folder_id, com_label_file, dannce_label_file, params, video_metadata
        )

        # 6. identify and write predictions to database
        ...
        logger.warning(f"Exception while uploading video folder: {e}")

        _update_db_failed(video_folder_id)


        logger.info(f"Runtime until failure: {ellapsed_seconds} s")
        ellapsed_seconds = time.time() - start
        return {
            "success": False,
            "video_folder_id": video_folder_id,
...
def _copy_reencode_video_folder(src: Path, dest: Path):
    ...
    for i, [src_file, dest_file] in enumerate(
        _enumerate_video_files(src_path, dest_path)
    ):
...
def _identify_copy_calibration_params(
    video_folder_path_src: Path, video_folder_path_dest: Path
) -> IdentifyCalibrationParamsReturn:
    ...
    latest_com_label = max(
        com_label_files_info, key=lambda x: x.timestamp, default=None
    )

    latest_dannce_label = max(
        dannce_label_files_info, key=lambda x: x.timestamp, default=None
    )
    ...
    if latest_dannce_label:
        logger.info(
            f"Using params from DANNCE labels file: {latest_dannce_label.filename}"
        )
    ...
    if not params:
        raise Exception(
            f"Unable to locate params for video folder: {str(video_folder_path_src)}"
        )
    ...
    return {
        ...
        "dannce_label_file": Path(latest_dannce_label.filename)
        if latest_dannce_label
        else None,
    }
...
def _identify_and_copy_prediction_files(
    src_video_folder_path: Path,
) -> list[PredictionInfo]:
    predictions: list = []
    ...
    for src_path in dannce_data_paths:
        ...
        dest_folder_name = make_resource_name("DANNCE_")
    ...
    for src_path in com_data_paths:
        src_folder_name = src_path.parent.name
        dest_folder_name = make_resource_name("COM_")
        dest_folder_path = settings.PREDICTIONS_FOLDER.joinpath(dest_folder_name)
        dest_folder_path.mkdir()
...
def _update_db_failed(
    video_folder_id,
):
    with get_db_context() as conn:
        curr = conn.cursor()
        curr.execute("BEGIN")
        curr.execute(f"UPDATE {TABLE_VIDEO_FOLDER} SET status='FAILED' WHERE id=?", (video_folder_id,))
        curr.execute("COMMIT")
    logger.warning(f"Failed to complete video folder with id: {video_folder_id}. See logs for more details.")



def _update_db_completed(
    video_folder_id, com_data_file, dannce_data_file, params_jsonable, video_metadata
):
    with get_db_context() as conn:
        curr = conn.cursor()
        curr.execute("BEGIN")
...
WHERE id=?
        ...
        curr.execute("COMMIT")
        logger.info(f"UPDATED VIDEO FOLDER WITH ID: {video_folder_id}")


def _write_predictions_to_db(predictions: list[PredictionInfo], video_folder_id: int):
    prediction_ids = []
    with get_db_context() as conn:
        curr = conn.cursor()
        curr.execute("BEGIN")
